%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [model,k] = scaleLipidsRhto(model,scaling)
%
...
function [model,k] = scaleLipidsRhto(model,scaling)
...
if block
    ...
    model = setParam(model,'eq',[backbExIdx,chainExIdx],[0,0]);
end
...
function [sol,model] = solveRhto(model)
model = setParam(model,'lb',{'r_1714','r_2111'},[-1,0.001]);
bmIdx = getIndexes(model,'r_2111','rxns');
model = setParam(model,'obj','r_2111',1);
sol   = solveLP(model);
model = setParam(model,'eq','r_2111',sol.x(bmIdx));
